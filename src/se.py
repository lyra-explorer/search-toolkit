...
def cmd_init(args) -> None:
    ...
    # YAML生成（pyyamlがなくても書けるように手書き）
    lines = [
        "# se config — auto-generated by 'se init'",
        f"# Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        "",
        "defaults:",
        f'  search_root: "{_yaml_esc(DEFAULT_PATH)}"',
        "  max_results: 100",
    ...
    lines.append("    paths:")
    lines.append(f'      - "{_yaml_esc(DEFAULT_PATH)}"')
...
def es_search(regex: str, path: str | None, n: int | None) -> list[str]:
    cmd = [ES, "-r", regex]
    cmd += ["-path", path] if path else ["-path", DEFAULT_PATH]
    if n is not None:
        cmd += ["-n", str(n)]
...
def cmd_doctor(args) -> None:
    ...
    _check("pymigemo",
           ...
           "pymigemo がインストールされていません: pip install pymigemo", issues)
...
def _check(name: str, fn, error_msg: str | None, target: list) -> None:
...
def _fix_es_not_found() -> str | None:
    if not Path(ES).exists():
        ...
        for c in candidates:
            if Path(c).exists():
                return f"es.exe 発見: {c} — se.py の ES 定数を更新してください"
        return None
